import time, pathlib, sys, datetime, threading, json, os, shutil, wget
import minio
...
class LocalFileHandler:
    ...
    def list(self, extention=""):
        return find_all_files_in_folder(shared_data_dir, extension=extention)

    def copy_to_shared_folder(self,source_file_path):
        if file_exists(source_file_path):
            dst_folder_path = os.path.join(shared_data_dir,self.destination_prefix)
            os.makedirs(dst_folder_path,exist_ok=True)

            dst_file_path = os.path.join(dst_folder_path, os.path.split(source_file_path)[1])
            ...
            logger.error(f"{source_file_path}  does not exist, Could not copy {source_file_path} to {dst_file_path}")
            return None

    def download_file_from_url(self, url:str, output_file_path:str=""):
        def bar_custom(current, total, width=80):
            print("Downloading: %d%% [%d / %d] bytes" % (current / total * 100, current, total))
        wget.download(url, bar=bar_custom, out=output_file_path)
