...
class DtccCitySolarIrradiance(RunInShell,PubSubBase):
    ...
    def run_command(self, parameters:dict) -> str:
        ...
        inputfile_path = os.path.join(self.local_data_dir, prefix, file_name)
        latitude = parameters.get("latitude", None)
        longitude = parameters.get("longitude", None)
        date = parameters.get("date", None)

        shell_command = f"python3 -m citysolar.RunSolar --inputfile {inputfile_path} --latitude {latitude} --longitude {longitude} --one_date {date} --exportpath {self.output_file_name}"
